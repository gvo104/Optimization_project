...
## 8. Статистическое сравнение методов

Каждый алгоритм запускался **30 раз** независимо. Критерий сравнения — значение целевой функции Φ в конце работы (чем меньше, тем лучше).

### Описательная статистика

| Метод | Mean   | Std   | Best   | Worst | Median |
|-------|--------|-------|--------|-------|--------|
| GA    | −63.00 | 56.09 | −85.00 | 80.00 | −85.00 |
| PSO   | −46.50 | 69.79 | −85.00 | 80.00 | −85.00 |
| ACO   | −85.00 |  0.00 | −85.00 | −85.00 | −85.00 |
| SA    |  −8.00 | 82.32 | −85.00 | 80.00 | −85.00 |

**ACO** — единственный метод с нулевым разбросом: находит глобальный оптимум в 100% запусков.

### Попарный тест Манна–Уитни (α = 0.05, поправка Бонферрони α' ≈ 0.0083)

Ячейка на пересечении строки M1 и столбца M2 — p-value гипотезы «M1 даёт меньшие значения Φ, чем M2». Звёздочка `*` = значимо лучше.

|     | GA     | PSO    | ACO    | SA      |
|-----|--------|--------|--------|---------|
| GA  | —      | 0.1632 | 0.9808 | 0.0027* |
| PSO | 0.8422 | —      | 0.9975 | 0.0308  |
| ACO | 0.0209 | 0.0027*| —      | 0.0000* |
| SA  | 0.9975 | 0.9705 | 1.0000 | —       |

### Вывод

**Победитель: ACO** — статистически значимо лучше PSO и SA; от GA неотличим (p = 0.0209, выше порога Бонферрони), однако GA не лучше ACO (p = 0.98). Среди методов с одинаковой медианой ACO выигрывает по среднему (−85.00 против −63.00 у GA) и разбросу (std = 0 против 56).

SA показал наихудшую стабильность: высокий std означает, что результат сильно зависит от начального состояния и параметров охлаждения.

### График распределений

![statistical_comparison](output/statistical_comparison.png)

*Ящики с усами по 30 запускам. ACO вырождается в точку — оптимум гарантирован при любом запуске.*
...
- Число повторных запусков для статистики задаётся в `main.py`: `run_statistical_comparison(config, n_runs=30)`.